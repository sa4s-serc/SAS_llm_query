...
class BookingService:
    ...
    def _book_location(self):
        ...
        try:
            conn = sqlite3.connect('booking_data.db')
            cursor = conn.cursor()
            cursor.execute('''INSERT INTO Bookings (location, start_time, end_time) VALUES (?, ?, ?)''',
    ...
    def _check_availability_internal(self, data):
        ...
        try:
            ...
            conn.close()
            if count > 0:
                return {"status": "error", "message": "Location is already booked for the requested time"}
            else:
    ...
    def _check_quality_internal(self, data):
        ...
        try:
            ...
            #     if not sensor_name:
            #         return {"status": "error", "message": f"Invalid sensor type: {sensor}"}
