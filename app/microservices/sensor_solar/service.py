...
logger = setup_logger("SensorSolarService")
...
class SensorSolarService(MicroserviceBase):
    def __init__(self):
        super().__init__("sensor_solar_service")
        self.update_service_info(
            description="Handles solar sensor data and notifications",
    ...
    async def _handle_notification(self, data: SensorData):
        try:
            self._store_to_mongodb_sensor(
                db_name="solar_sensor_db",
                collection_name="solarsensordata",
    ...
    async def _get_data(self, id: str):
        try:
            data = self._fetch_from_mongodb("solar_sensor_db", "solarsensordata", id)
            if data:
...
def start_sensor_solar_service():
    service = SensorSolarService()
    service.run()


if __name__ == "__main__":
    start_sensor_solar_service()
