...
logger = setup_logger("SensorCrowdService")
...
class SensorCrowdService(MicroserviceBase):
    def __init__(self):
        super().__init__("sensor_crowd_service")
        self.update_service_info(
            description="Handles crowd sensor data and notifications",
    ...
    async def _handle_notification(self, data: SensorData):
        try:
            self._store_to_mongodb_sensor(
                db_name="crowd_sensor_db",
                collection_name="crowdsensordata",
    ...
    async def _get_data(self, id: str):
        try:
            data = self._fetch_from_mongodb("crowd_sensor_db", "crowdsensordata", id)
            if data:
