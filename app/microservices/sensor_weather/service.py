...
logger = setup_logger("SensorWeatherService")
...
class SensorWeatherService(MicroserviceBase):
    def __init__(self):
        super().__init__("sensor_weather_service")
        self.update_service_info(
            description="Handles weather sensor data and notifications",
    ...
    async def _get_data(self, id: str):
        try:
            data = self._fetch_from_mongodb(
                "weather_sensor_db", "weathersensordata", id
            )
            if data:
...
def start_sensor_weather_service():
    service = SensorWeatherService()
    service.run()


if __name__ == "__main__":
    start_sensor_weather_service()
