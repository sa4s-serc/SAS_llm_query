...
class WaterQualityService(MicroserviceBase):
    def __init__(self):
        ...
        self.update_service_info(
            description="Service to monitor water quality metrics in various locations",
            dependencies=[]
        )
    ...
    def process_request(self, location: str, timestamp: Optional[str]):
        ...
        if not filtered_data:
            raise HTTPException(status_code=404, detail="No data for the specified timestamp")
...
if __name__ == '__main__':
    start_water_quality_service().run()
