...
class TravelService(MicroserviceBase):
    ...
    def register_routes(self):
        ...
        def get_travel_options(option: TravelOption):
    ...
    def process_request(self, option: TravelOption):
        ...
        for item in self.data:
            if (
                (not option.destination or item['destination'] == option.destination) and
                (not option.available_time or item['available_time'] == option.available_time) and
                (not option.preferred_mode or item['preferred_mode'] == option.preferred_mode)
            ):
