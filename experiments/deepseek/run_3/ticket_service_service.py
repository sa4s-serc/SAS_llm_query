...
class TicketRequest(BaseModel):
    ...
    price_ranges: Optional[List[List[int]]]
...
class TicketService(MicroserviceBase):
    ...
    def register_routes(self):
        @self.app.post("/tickets", response_model=List[TicketResponse])
        async def get_tickets(request: TicketRequest):
            ...
            if request.price_ranges:
                for price_range in request.price_ranges:
                    if len(price_range) == 2 and price_range[0] <= ticket_price <= price_range[1]:
                        ...
                        break
            if available:
                results.append(TicketResponse(event_name=event_name, ticket_price=ticket_price, available=available))
        return results
