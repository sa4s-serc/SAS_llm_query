...
class HistoricSiteRequest(BaseModel):
    site_names: List[str]

class HistoricSiteResponse(BaseModel):
    name: str
    year_built: str
    significance: str
...
class HistoricDataService(MicroserviceBase):
    ...
    def register_routes(self):
        @self.app.post("/historic-data", response_model=List[HistoricSiteResponse])
        async def get_historic_data(request: HistoricSiteRequest):
            return self.process_request(request)

    def process_request(self, request: HistoricSiteRequest):
        results = []
